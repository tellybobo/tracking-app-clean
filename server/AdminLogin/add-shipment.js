document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (trackingNumber) {
        try {
            ...
            const response = await fetch(`https://backend-server-km7h.onrender.com/api/shipments/${trackingNumber}`);
    ...
    async function handleFormSubmission(form, originalTracking) {
        ...
        try {
            ...
            const response = await fetch('https://backend-server-km7h.onrender.com/api/shipments', {
                method: 'POST',
                headers: { 
